...
- `EP_PE结果统计分析.tex/.pdf`：修改前的 LaTeX 统计快照；新口径定稿前以 Markdown 主文档和 Parquet 结果为准。
- `EP_PE统计分析与因子测试计划.md`：便于实时预览和直接修改的 Markdown 主文档；公式采用兼容所有预览器的纯文本数学格式，行业图引用 `图形\industry_pe_latest.png`。
- `EP_PE统计分析与因子测试计划.tex/.pdf`：修改前的合并版快照，待 Markdown 内容定稿后重新生成。
- `EP_PE结果统计简版.tex`：修改前的一页 LaTeX 正文快照，待 Markdown 定稿后更新。
- `行业PE柱状图.tex`：使用 `pgfplots` 绘制的申万一级行业原始 PE 柱状图正文片段。
- `中间结果\industry_pe_latest.csv`：行业柱状图对应的统计明细。
- `图形\industry_pe_latest.png`：同口径的 PNG 预览图。
- `回归结果\RLM_新口径\RLM统计结果.md`：新口径 EP 月度横截面 Huber RLM 的完整统计结果。
...
89 只 B 股过滤、北交所代码映射和公告版本防回退口径更新。现有 `.tex/.pdf` 是本轮修改前的快照，待
Markdown 定稿后再统一转换和编译。

目录中的旧月度 OLS 和单日 OLS 文件均早于本次口径修正，仍不能作为新口径的回归结论。
本次已使用 `monthly_rlm_ep.py` 完成 64 个月度横截面 Huber RLM，正式结果位于
`回归结果\RLM_新口径`；尚未运行 OLS/WLS 对照、IC 或分层回测。
...
`pgfplots` 片段。当前截面为 `2025-12-31`，31 个行业共使用 5,438 条行业和 PE
...
6. 本次已运行新口径 RLM，但没有运行 OLS/WLS 对照、IC 或分层回测；`回归结果` 根目录中的旧 OLS 文件仍为历史结果。
